...
export async function POST(req: NextRequest) {
  ...
  const upload = async (key: "base" | "left" | "right") => {
    const override = key === "base" ? baseOverride : key === "left" ? leftOverride : rightOverride;
  ...
  if (!baseUrl && !leftUrl && !rightUrl) {
    return NextResponse.json({ success: false, message: "Provide at least one lining image" }, { status: 400 });
  }
