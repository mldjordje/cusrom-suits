...
const SuitPreview: React.FC<Props> = ({ config }) => {
  ...
  const torsoLayersOrig = baseLayers.filter((l) => l.id !== "pants");
  ...
  const fabricStyle = (
    ...
    if (adjust?.brightness) style.filter = `${style.filter} brightness(${adjust.brightness})`;
    ...
    if (adjust?.blendMode) style.mixBlendMode = adjust.blendMode;
    return style;
  };
  ...
  return (
    ...
    >
      <div className="relative w-[360px] md:w-[520px] aspect-[2/3] mb-[-40px]" style={{ transform: `translate(${offset.x}px, ${offset.y}px) scale(${scale})`, transformOrigin: "center" }}>
        ...
        {(() => {
          const maskSrcs = [
            ...torsoLayersOrig.map((l) => l.src), // original first (fallback)
            ...torsoLayers.map((l) => l.src),     // dynamic next
            ...(lapelSrc ? [lapelSrc] : []),
          ];
          ...
          const unionKey = `fabric-union-${maskList}-${fabricTexture}-${config.lapelId ?? 'none'}-${config.lapelWidthId ?? 'auto'}`;
