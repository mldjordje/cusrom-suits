...
// Fabric overlay tuning (kept bright); shading above adds depth
const toneOverlay = (tone: string) => {
  switch (tone) {
    case "light":
      return { opacity: 1, blendMode: "normal" as const, filter: "brightness(1.02) contrast(1.02)" };
    case "dark":
      return { opacity: 1, blendMode: "normal" as const, filter: "brightness(1.04)" };
    default:
      return { opacity: 1, blendMode: "normal" as const, filter: "brightness(1.02) contrast(1.02)" };
  }
};

// Depth constants for multiply shading
const SHADE_OPACITY = 0.58;
const SHADE_FILTER = "grayscale(1) contrast(1.18) brightness(0.96)";
...
const SuitPreview: React.FC<Props> = ({ config }) => {
  ...
  // Ensure torso sprite path matches selected lapel (type/width)
  ...
  return (
    ...
    >
      ...
      >
        {/* Interior (use transparent silhouettes so fabric does not affect) */}
        ...
        {/* Union fabric overlay for jacket (torso+sleeves+bottom+lapel) → no seams */}
        {(() => {
          ...
          return <div className="absolute inset-0" style={unionStyle} />;
        })()}

        {/* Per-layer shading on top for depth (uses original color sprites) */}
        {torsoLayers.map((layer) => (
          <div key={layer.id} className="absolute inset-0">
            <Image src={layer.src} alt={layer.id} fill sizes="(max-width: 768px) 100vw, 520px" priority style={{ objectFit: "contain", pointerEvents: "none", mixBlendMode: "multiply", opacity: SHADE_OPACITY, filter: SHADE_FILTER }} />
          </div>
        ))}

        {shirtSrc && (
          <div className="absolute inset-0 z-10">
            <Image src={shirtSrc} alt="shirt" fill sizes="(max-width: 768px) 100vw, 520px" priority style={{ objectFit: "contain", pointerEvents: "none" }} />
          </div>
        )}

        {/* Hip pockets (fabric + shading) */}
        {pocketSrc && (
          <div className="absolute inset-0 z-10">
            <div className="absolute inset-0" style={fabricStyle(pocketSrc)} />
            <Image src={pocketSrc} alt="pockets" fill sizes="(max-width: 768px) 100vw, 520px" priority style={{ objectFit: "contain", pointerEvents: "none", mixBlendMode: "multiply", opacity: SHADE_OPACITY, filter: SHADE_FILTER }} />
          </div>
        )}

        {/* Breast pocket (fabric + shading) */}
        {breastPocketLayers && breastPocketLayers.map((layer) => (
          <div key={layer.id} className="absolute inset-0 z-20">
            ...
            <Image src={layer.src} alt={layer.name} fill sizes="(max-width: 768px) 100vw, 520px" priority style={{ objectFit: "contain", pointerEvents: "none", mixBlendMode: "multiply", opacity: SHADE_OPACITY, filter: SHADE_FILTER }} />
          </div>
        ))}

        {/* Lapel shading (fabric already comes from union overlay) */}
        {lapelSrc && (
          <div className="absolute inset-0 z-0">
            <Image src={lapelSrc} alt="lapel" fill sizes="(max-width: 768px) 100vw, 520px" priority style={{ objectFit: "contain", pointerEvents: "none", mixBlendMode: "multiply", opacity: SHADE_OPACITY, filter: SHADE_FILTER }} />
          </div>
        )}
      </div>
      ...
      {pantsLayer && (
        <div className="relative w-[540px] md:w-[760px] aspect-[3/1] mt-[-20px]" style={{ transform: `translate(${offset.x}px, ${offset.y}px) scale(${scale})`, transformOrigin: "center" }}>
          <div className="absolute inset-0">
            ...
            <Image src={pantsLayer.src} alt="pants" fill sizes="(max-width: 768px) 100vw, 760px" priority style={{ objectFit: "contain", pointerEvents: "none", mixBlendMode: "multiply", opacity: SHADE_OPACITY, filter: SHADE_FILTER }} />
          </div>

          {cuffSrc && (
            <div className="absolute inset-0">
              <div className="absolute inset-0" style={fabricStyle(cuffSrc)} />
              <Image src={cuffSrc} alt="cuffs" fill sizes="(max-width: 768px) 100vw, 760px" priority style={{ objectFit: "contain", pointerEvents: "none", mixBlendMode: "multiply", opacity: SHADE_OPACITY, filter: SHADE_FILTER }} />
            </div>
          )}
          ...
          {pantsPleatSrc && (
            <div className="absolute inset-0">
              ...
              <Image src={pantsPleatSrc} alt="pants-pleats" fill sizes="(max-width: 768px) 100vw, 760px" priority style={{ objectFit: "contain", pointerEvents: "none", mixBlendMode: "multiply", opacity: SHADE_OPACITY, filter: SHADE_FILTER }} />
            </div>
          )}
        </div>
      )}
