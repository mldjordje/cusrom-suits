...
type ManualOverride = {
  directions?: Partial<typeof PANTS_STRIPE_MANUAL_TUNING.directions>;
  boundaries?: Partial<typeof PANTS_STRIPE_MANUAL_TUNING.boundaries>;
  offsets?: Partial<typeof PANTS_STRIPE_MANUAL_TUNING.offsets>;
};

const getRuntimeOverride = (): ManualOverride | null => {
  const root = globalThis as Record<string, unknown>;
  const raw = root.__SANTOS_PANTS_STRIPE_TUNING_OVERRIDE;
  if (!raw || typeof raw !== "object") return null;
  return raw as ManualOverride;
};

const mergeManual = () => {
  const override = getRuntimeOverride();
  const directions = {
    ...PANTS_STRIPE_MANUAL_TUNING.directions,
    ...(override?.directions ?? {}),
  };
  const boundaries = {
    ...PANTS_STRIPE_MANUAL_TUNING.boundaries,
    ...(override?.boundaries ?? {}),
  };
  const offsets = {
    ...PANTS_STRIPE_MANUAL_TUNING.offsets,
    ...(override?.offsets ?? {}),
  };
  return { directions, boundaries, offsets };
};

export const getPantsStripeTuning = () => {
...
type PantsStripeTuning = ReturnType<typeof getPantsStripeTuning>;

export const PANTS_STRIPE_TUNING = new Proxy({} as PantsStripeTuning, {
  get(_target, prop) {
    return (getPantsStripeTuning() as Record<string, unknown>)[String(prop)];
  },
  ownKeys() {
    return Reflect.ownKeys(getPantsStripeTuning() as Record<string, unknown>);
  },
  getOwnPropertyDescriptor(_target, prop) {
    return {
      configurable: true,
      enumerable: true,
      value: (getPantsStripeTuning() as Record<string, unknown>)[String(prop)],
      writable: false,
    };
  },
});
