...
export function mapLegacyProductToAnanasImport(
  ...
  return {
    ...
    parentEan: ean,
    packageWeightValue: toFixedString(packageWeightValue, 2),
    packageWeightUnit,
    basePrice: toFixedString(product.price.gross, 2),
    vat: toFixedString(product.price.taxPercent, 2),
    stockLevel: Math.max(0, Math.floor(product.stock.warehouse1)),
    sku,
    externalId: product.legacyId,
